...
function showPersonSearchDialog() {
  if (!window.personSearchAPI) {
    console.error("Search system not ready");
    return;
  }
  ...
  modal.innerHTML = `
      <div style="background: white; width: 90%; max-width: 600px; height: fit-content;
                  border-radius: 8px; padding: 20px; position: relative;">
        <h3 style="margin-top: 0;">Tìm kiếm thành viên</h3>
        <div id="search-embed-area"></div>
  ...
  // Nhúng search container vào modal
  const searchContainer = document.querySelector(".global-search-container");
  const embedArea = document.getElementById("search-embed-area");

  if (searchContainer && embedArea) {
    // Reset style cho phù hợp với modal
...
// Xử lý sự kiện click menu item (giữ nguyên)
document
  .querySelectorAll('.search-option[data-type="person"]')
  .forEach((option) => {
    option.addEventListener("click", (e) => {
      e.preventDefault();
      showPersonSearchDialog();

      // Đóng submenu nếu là mobile
      if (window.innerWidth <= 768) {
        const submenu = option.closest(".search-submenu");
